...
> 以下内容仅对单卡情况下有效，多卡的时候共享GPU内存将不再被使用，每张显卡只能访问自己的专用GPU内存，并且pin_memory也会大幅缩水！一种已知的方案是，使用环境变量`export CUDA_VISIBLE_DEVICES=1`来迫使系统只能看到一张显卡，但是此时**似乎**可用的共享GPU内存为总共享GPU内存/独显个数。
{: .prompt-warning }
![alt text](/home/xjh/myblogs/assets/img/gpu-vram/control_nvidia.png)
_可以尝试改变英伟达控制面板的设置，来**增大**使用共享GPU内存的概率_
