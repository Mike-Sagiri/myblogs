...
因为WSL会在空闲时关闭，所以需要手动将空闲时关闭VM取消掉。

首先，打开WSL Settings图形化界面：
![alt text](assets/img/wsl-ssh/wsl_setting.png)
_wsl setting_
将**空闲超时**改成-1。

此外，如果需要Windows开机自动启动WSL，可以利用任务计划程序。

右键此电脑->显示更多选项->管理->任务计划程序->创建任务

按照如下进行配置：
![alt text](assets/img/wsl-ssh/first_schedule.png)
_任务计划程序首页_

![alt text](assets/img/wsl-ssh/trigger_schedule.png)
_任务计划程序触发器设置_

![alt text](assets/img/wsl-ssh/operation_schedule.png)
_任务计划程序操作设置，记得把发行版改成自己的_

![alt text](assets/img/wsl-ssh/condition_schedule.png)
_任务计划程序条件设置，红框笔记本尤其重要_

![alt text](assets/img/wsl-ssh/setting_schedule.png)
_任务计划程序设置，尤其注意红框，它是默认开启的，由于wsl被开启后会驻留着，开着的话三天后它会自动给wsl关掉_

## ssh到docker

docker容器也是一样的，在使用`--net host`参数启动后，就可以直接从局域网连接到docker容器了。

关于docker还有一个问题，如果Windows上安装了docker，并且开启了wsl integration之后，那么那个wsl系统的默认用户就能在wsl的终端访问docker了，但是其他用户是不行的。这个时候需要将其他用户加入用户组：
...
```
然后将其他用户加入docker用户组并刷新：
```bash
sudo usermod -aG docker 其他用户
newgrp docker
```
以上都需要在默认用户中运行。
...
<blockquote class="prompt-warning">
  <p>已知Windows Docker Desktop+WSL(Mirror Net Mode) Integration+ssh会有一个问题，在docker启动后正常一段时间之后，wsl的systemd会出问题，具体表现为systemctl is-system-running会光标闪烁无输出，以及其他相关程序全类似表现。建议wsl内部安装一个linux版本的docker，是最稳定最隔离的方案，并且事实上Windows上的docker也是wsl后端。</p>
</blockquote>